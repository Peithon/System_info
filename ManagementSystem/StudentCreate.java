package ManagementSystem;
	...
	import javax.swing.*;
	public class StudentCreate extends JFrame implements ActionListener {
	     ...
	     private  JLabel picture;                                 // use a label to display the images
		 ...
		 public StudentCreate(String string ,Connection conn,PreparedStatement perstmt){
			 this.conn=conn;
		     ...
		     panel8.add(picture,BorderLayout.CENTER);
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(arg0.getSource().equals(button2)){       //退出
				...
				if(fiel1.getText().isEmpty()&&fiel2.getText().isEmpty()&&fiel3.getText().isEmpty()){
					JOptionPane.showMessageDialog(null,"请填写学生资料","警告",JOptionPane.WARNING_MESSAGE);
	            ...
	            if(!(fiel1.getText().isEmpty()||fiel2.getText().isEmpty()||fiel3.getText().isEmpty())){                                      //把资料送入数据库
					try{     					
					     ...
					     sql = "INSERT INTO S(sno,sname,sex,dept,age) VALUES(?,?,?,?,?)";
				            ...
				            conn.commit();	
				         sql=" insert into student(username,password) values(?,'123456')";
				            perstmt=conn.prepareStatement(sql);
				            perstmt.setString(1,fiel1.getText()); 
				            perstmt.executeUpdate();
				            conn.commit();	
